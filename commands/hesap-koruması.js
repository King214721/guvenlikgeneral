...
module.exports.execute = async (client, interaction, db) => {
    ...
    if(interaction.options.getString("durum") === "true") {
        ...
        const embed = new Discord.EmbedBuilder()
        .setColor("#ED4245")
        .setDescription("> **Hesap koruması koruması zaten aktif.** <:cs_reddet:1142405174548254802>")

        if(db.fetch(`korumaLog.guardAccountUser_${interaction.guild.id}`)) return interaction.reply({ embeds: [embed], ephemeral: true })

        const embed1 = new Discord.EmbedBuilder()
        .setColor("#57F287")
        .setDescription("> **Hesap koruması koruması `aktif` edilidi!** <:cs_onay:1142405258409156708>")
        ...
        const embed = new Discord.EmbedBuilder()
        .setColor("#ED4245")
        .setDescription("> **Hesap koruması koruması zaten deaktif.** <:cs_reddet:1142405174548254802>")

        if(!db.fetch(`korumaLog.guardAccountUser_${interaction.guild.id}`)) return interaction.reply({ embeds: [embed], ephemeral: true })

        const embed1 = new Discord.EmbedBuilder()
        .setColor("#57F287")
        .setDescription("> **Hesap koruması koruması `deaktif` edilidi!** <:cs_onay:1142405258409156708>")
        .setFooter({ text:  `${interaction.user.tag} tarafından istendi.`, iconURL: interaction.user.displayAvatarURL({ dynamic: true })})
        .setTimestamp()
