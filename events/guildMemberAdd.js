...
module.exports = async(client, member) => {
    ...
    if(botKoruması) {
        ...
        if(member.user.bot) {
            ...
            .setDescription("<:cs_warn2:1142138650314952795>  **"+member.user.tag+"** kara listede bulunduğu için sunucdan atıldı.\n```/bot-koruması izni "+member.user.id+"```")
    ...
    if(adminKoruması) {
      ...
      if(member.permissions.has(PermissionsBitField.Flags.Administrator)) {
        ...
        const embed = new EmbedBuilder()
        .setColor("#FEE75C")
        .setDescription("<:cs_warn2:1142138650314952795>  **"+member.user.tag+"** yönetici izinli giriş yaptığı için sunucudan atıldı.")
    ...
    if(hesapKoruması) {
        ...
        if(now) {
			...
			member.kick();
				log.send({ 
					embeds: [
					new EmbedBuilder().setDescription(`<:cs_warn2:1142138650314952795>  **${member.user.tag}**, Hesabı yeni olduğu için sunucudan atıldı.`).setColor(`#FEE75C`).setFooter({ text: `${member.user.tag}`, iconURL: member.user.displayAvatarURL({ dynamic: true }) })
					]
				 })
